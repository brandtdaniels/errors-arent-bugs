...
When a product team defines a feature, they describe the happy path in detail. "The user taps Download, a progress bar appears, the map is saved for offline use." What happens when the download fails? What happens when the device runs out of storage? What happens when the user loses connectivity midway through? These are not edge cases. They are foreseeable, common conditions. But they are rarely specified.
...
Errors, on the other hand, are expected. Networks time out. Servers return 500s. Users lose GPS signal in a canyon. Disks fill up. Sessions expire. These are not surprises. They are known, predictable conditions that every application will encounter.
...
- A map tile fails to load, leaving a blank area with no indication anything is missing.
...
**An important design note:** generic error messages and leaked technical details are the same maturity level. "Something went wrong" and "NullPointerException at com.example.service.MapService.java:142" are both dead ends from the user's perspective. One is masked, one is leaked, but neither helps.

**Examples:**

- "An error occurred. Please try again later."
- "Error: NullPointerException at com.example.service.MapService.loadTile(MapService.java:142)"
...
- "Unable to download map. (Error MAP-1042)" — support can immediately look up this code and guide the user.
...
- "This area couldn't be loaded. Would you like to download it for offline use?"
...
- "GPS signal lost. Showing your last known location. We'll update automatically when signal is restored."
...
- **Graceful degradation:** The app offers reduced functionality rather than full failure. Offline modes, cached data, and fallback workflows keep the user productive.
...
Each layer of the application catches errors, wraps them with context, and passes them upward. The network layer knows the request timed out. The service layer knows it was a tile fetch for a specific region. The UI layer knows the user was trying to pan to a new area.
